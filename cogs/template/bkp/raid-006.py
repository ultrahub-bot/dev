...
class RaidBuilderSystem(commands.Cog):
        ...
        if len(raid['members']) >= raid['party_size']:
            ...
            try:
                await member.send(
                    ...
                    f"Você será movido para fuera do canal."
                )
            except:
                pass
    ...
    async def cleanup_raid(self, raid_id: int):
        ...
        # Apagar canal de voz se for uma raid de voice queue
        ...
        # CORREÇÃO: deletar mensagem original
        ...
        # CORREÇÃO: deletar thread
        if raid_id in self.raid_threads:
            thread_id = self.raid_threads[raid_id]
            thread = self.bot.get_channel(thread_id)
            if thread:
                try:
                    await thread.delete(reason=f"Raid {raid_id} concluída")
                except Exception as e:
    ...
    async def send_raid_embed(self, raid_data: Dict, channel: discord.TextChannel):
        ...
        try:
            ...
            # Criar thread para a raid
            thread_name = f"🎯 {boss['name']} - {leader.display_name}"
            thread = await message.create_thread(
                name=thread_name[:100],  # Limite de caracteres
            ...
            await thread.send(
                ...
                f"💬 Use esta thread para coordenar a raid!\n"
                f"🎮 Entre na raid usando os botões na mensagem acima!"
            )
    ...
    def create_raid_view(self, raid_data: Dict) -> View:
        """Versão corrigida da criação da view"""
        view = View(timeout=None)  # Sem timeout para persistência
        ...
        if raid_data['status'] == 'recruiting':
            ...
            join_btn.callback = lambda i: self.handle_join_raid(i, raid_data['id'])
            view.add_item(join_btn)

            if not raid_data['is_voice_queue']:
                invite_btn = Button(
                    style=ButtonStyle.blurple,
                    label="📨 Convidar",
                    custom_id=f"raid_invite_{raid_data['id']}"
                )
                invite_btn.callback = lambda i: self.handle_invite_modal(i, raid_data['id'])
                view.add_item(invite_btn)

            ready_btn = Button(
                style=ButtonStyle.gray,
                label="✅ Ready Check",
                custom_id=f"raid_ready_{raid_data['id']}"
            )
            ready_btn.callback = lambda i: self.handle_ready_check(i, raid_data['id'])
        ...
        # ──────────────── STATUS: READY CHECK ────────────────
            ...
            confirm_btn.callback = lambda i: self.handle_confirm_ready(i, raid_data['id'])
            view.add_item(confirm_btn)

            cancel_ready_btn = Button(
                style=ButtonStyle.red,
                label="❌ Cancelar Ready Check",
                custom_id=f"raid_cancel_ready_{raid_data['id']}"
            )
            cancel_ready_btn.callback = lambda i: self.handle_cancel_ready(i, raid_data['id'])
            view.add_item(cancel_ready_btn)

            # Se todos confirmaram → só adiciona o botão "Concluir"
            if len(raid_data['confirmed_members']) == len(raid_data['members']):
                finish_btn = Button(
                    style=ButtonStyle.success,
                    label="🏆 Concluir Raid",
                    custom_id=f"raid_finish_{raid_data['id']}"
                )
                ...
                async def finish_callback(interaction: discord.Interaction):
                    ...
                    raid_data['status'] = 'completed'
                    raid_data['completed_at'] = datetime.utcnow()
                    await self.update_raid_embed(raid_data['id'])
                    await self.cleanup_raid(raid_data['id'])
        ...
        # ──────────────── BOTÃO CANCELAR RAID ────────────────
        # (aparece tanto em recruiting quanto em ready_check)
        if raid_data['status'] in ['recruiting', 'ready_check']:
            cancel_raid_btn = Button(
                style=ButtonStyle.danger,
                label="🚫 Cancelar Raid",
                custom_id=f"raid_cancel_{raid_data['id']}"
            )
            cancel_raid_btn.callback = lambda i: self.handle_cancel_raid(i, raid_data['id'])
    ...
    async def update_raid_embed(self, raid_id: int):
        """Versão corrigida da atualização do embed"""
        if raid_id not in self.active_raids:
            return
        
        raid_data = self.active_raids[raid_id]
        message_info = self.raid_messages.get(raid_id)
    ...
    async def handle_join_raid(self, interaction: discord.Interaction, raid_id: int):
        """Versão corrigida do handler de entrada na raid"""
        if raid_id not in self.active_raids:
            return await interaction.response.send_message(
        ...
        await self.send_to_raid_thread(
            raid_id,
            f"🎉 {interaction.user.mention} entrou na raid!\n"
        ...
        # Verificar se a raid ficou cheia
        if len(raid['members']) >= raid['party_size']:
            await self.handle_raid_filled(raid_id)
    ...
    async def handle_confirm_ready(self, interaction: discord.Interaction, raid_id: int):
        """Versão corrigida da confirmação de ready"""
        if raid_id not in self.active_raids:
            return await interaction.response.send_message(
                "❌ Esta raid não existe mais!",
                ephemeral=True
        ...
        if interaction.user.id in raid['confirmed_members']:
            return await interaction.response.send_message(
        ...
        await self.send_to_raid_thread(
            raid_id,
            f"✅ {interaction.user.mention} confirmou presença!\n"
        ...
        # ⚠️ Atualizar embed para refletir a nova contagem
        await self.update_raid_embed(raid_id)

        # Se todos confirmaram, atualizar embed e exibir botões de concluir/cancelar
        if len(raid['confirmed_members']) == len(raid['members']):
            await self.update_raid_embed(raid_id)
            await self.send_to_raid_thread(
                raid_id,
                "🎉 **Todos confirmaram!**\n"
                "O líder agora pode **🏆 Concluir** ou **🚫 Cancelar** a raid."
            )
    ...
    async def handle_cancel_raid(self, interaction: discord.Interaction, raid_id: int):
        """Versão corrigida do cancelamento de raid"""
        if raid_id not in self.active_raids:
            return await interaction.response.send_message(
                "❌ Esta raid não existe mais!",
                ephemeral=True
    ...
    async def send_to_raid_thread(self, raid_id: int, message: str, mention_role: bool = False):
        ...
        try:
            ...
            if mention_role:
                # Mencionar @everyone se permitido
    ...
    async def handle_ready_check_expired(self, raid_id: int):
        ...
        if confirmed_count >= max(2, total_members * 0.7):  # Pelo menos 2 ou 70%
            # Iniciar raid
