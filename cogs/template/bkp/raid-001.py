...
class RaidBuilderSystem(commands.Cog):
    ...
    async def handle_raid_filled(self, raid_id: int):
        """Handle quando uma raid fica cheia via canal de voz"""
        ...
        if channel:
            ...
            if mentions:
                await channel.send(
                    ...
                    f"{' '.join(mentions)}\n"
                    f"✅ Confirmem presença para iniciar a raid!"
                )
        
        # Iniciar ready check automático após 10 segundos
    ...
    async def start_auto_ready_check(self, raid_id: int):
        ...
        voice_channel = self.bot.get_channel(self.voice_queue_channels.get(raid['boss_id']))
    ...
    async def raid_builder(self, ctx: discord.ApplicationContext):
        """Interface principal do Raid Builder - REQUER estar no canal de voz específico"""
    ...
    async def show_boss_selection(self, interaction: discord.Interaction, mode: str):
        """Mostra seleção de bosses baseada no modo escolhido"""
        try:
        ...
        # Filtrar bosses baseado no modo
        ...
        async def boss_select_callback(select_interaction: discord.Interaction):
            ...
            # Correção do parsing: dividir corretamente o valor
            boss_id_mode = select.values[0]
            parts = boss_id_mode.split('_')
            boss_id = int(parts[0])  # Primeira parte é o ID do boss
            selected_mode = '_'.join(parts[1:])  # Resto é o modo
            
            boss = db.get_boss(boss_id)
            
            # Direcionar para o handler apropriado
            if selected_mode == 'voice_queue':
                await self.handle_voice_queue_creation(select_interaction, boss)
            elif selected_mode == 'private':
    ...
    async def create_raid(
        ...
        try:
            ...
            logger.info(f"Raid {raid_id} created for boss {boss_id} by user {leader_id}")
    ...
    async def create_raid_view(self, raid_id: int) -> View:
        """Cria view com botões para uma raid"""
        if raid_id not in self.active_raids:
            return View()
    ...
    async def start_ready_check(self, raid_id: int, interaction: discord.Interaction):
        """Inicia ready check para uma raid"""
        if raid_id not in self.active_raids:
            return await interaction.response.send_message(
                "❌ Esta raid não existe mais!",
                ephemeral=True
        ...
        boss = db.get_boss(raid['boss_id'])
        for user_id in raid['members']:
            if user_id != interaction.user.id:  # Não notificar quem iniciou
                user = self.bot.get_user(user_id)
                if user:
                    try:
                        await user.send(
                            ...
                            f"⏰ Confirme sua presença nos próximos 2 minutos."
                        )
                    except:
                        pass
    ...
    async def start_raid(self, raid_id: int):
        """Inicia a raid (quando todos confirmam no ready check)"""
        if raid_id not in self.active_raids:
            return
        ...
        for user_id in raid['members']:
            ...
            if user:
                ...
                try:
                    await user.send(
                        f"🎉 **RAID INICIADA!**\n"
                        f"Boa sorte contra **{boss['name']}**!\n"
                        f"🕒 Iniciada às: {raid['started_at'].strftime('%H:%M')}"
                    )
                except:
                    pass
        ...
        if channel and mentions:
            await channel.send(
                f"🎯 **Raid iniciada para {boss['name']}!**\n"
                f"{' '.join(mentions)}\n"
                f"🏆 Boa sorte e bom loot!"
            )
        
        await self.update_raid_embed(raid_id)
    
    async def complete_raid(self, raid_id: int, success: bool = True):
        """Marca uma raid como completada"""
        if raid_id not in self.active_raids:
            return
    ...
    async def cleanup_raid(self, raid_id: int):
        ...
        logger.info(f"Raid {raid_id} cleaned up")
    ...
    async def cleanup_task(self):
        ...
        for raid_id, raid in list(self.active_raids.items()):
            # Limpar raids que ficaram muito tempo recrutando
            if raid['status'] == 'recruiting' and (now - raid['created_at']).total_seconds() > 3600:  # 1 hora
            ...
            # Limpar ready checks expirados
                ...
                # Cancelar ready check se expirou
                raid['ready_check_started'] = False
                raid['confirmed_members'].clear()
                await self.update_raid_embed(raid_id)
                ...
                if leader:
                    try:
                        await leader.send(
                            ...
                            f"Nem todos confirmaram presença para **{boss['name']}**."
                        )
                    except:
                        pass
    ...
    async def voice_monitor(self):
        ...
        for boss_id, voice_channel_id in list(self.voice_queue_channels.items()):
            ...
            for user_id in raid['members']:
                user = self.bot.get_user(user_id)
                if user and user.voice and user.voice.channel.id == voice_channel_id:
                    continue
                
                # Usuário não está no canal, remover da raid
    ...
    async def ready_check_monitor(self):
        ...
        for raid_id, raid in list(self.active_raids.items()):
            if raid['ready_check_started'] and raid['ready_check_expires'] and now > raid['ready_check_expires']:
                ...
                # Verificar se pelo menos metade confirmou
                if len(raid['confirmed_members']) >= len(raid['members']) // 2:
                    await self.start_raid(raid_id)
                else:
                    ...
                    if leader:
                        try:
                            await leader.send(
                                f"❌ **Ready check falhou!**\n"
                                f"Não houve confirmações suficientes para **{boss['name']}**."
                            )
                        except:
                            pass
