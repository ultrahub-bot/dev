...
class RaidBuilderSystem(commands.Cog):
    ...
    async def send_raid_embed(self, raid_data: Dict, channel):
        """Envia o embed da raid e cria uma thread para ela"""
        try:
            ...
            await self.create_raid_thread(raid_data['id'], boss['name'])
    ...
    async def create_raid_thread(self, raid_id: int, boss_name: str) -> Optional[discord.Thread]:
        """Cria uma thread para organizar as mensagens da raid"""
        try:
            raid_channel = self.bot.get_channel(RAID_CHANNEL_ID)
            if not raid_channel or not isinstance(raid_channel, discord.TextChannel):
                logger.warning(f"Canal de raid {RAID_CHANNEL_ID} não é um canal de texto válido")
                return None
            
            # Verificar se o canal suporta threads
            if not raid_channel.permissions_for(raid_channel.guild.me).create_public_threads:
                logger.warning(f"Bot não tem permissão para criar threads no canal {RAID_CHANNEL_ID}")
                return None
            
            # Criar thread com nome descritivo
            thread = await raid_channel.create_thread(
                name=f"🎯 {boss_name} - Raid {raid_id}",
                type=discord.ChannelType.public_thread,
                reason=f"Thread para organização da raid {raid_id}"
            )
            
            # Salvar referência da thread
            self.raid_threads[raid_id] = thread.id
            
            # Mensagem inicial da thread
            embed = Embed(
                title=f"🎯 RAID {boss_name}",
                description=f"Esta thread foi criada para organizar a comunicação da raid #{raid_id}",
                color=Color.blue()
            )
            embed.set_footer(text="Todas as notificações e comunicações serão feitas aqui")
            
            await thread.send(embed=embed)
            
            logger.info(f"Thread {thread.id} criada para raid {raid_id}")
            return thread
            
        except Exception as e:
            logger.error(f"Erro ao criar thread para raid {raid_id}: {e}")
            return None

    async def send_to_raid_thread(self, raid_id: int, content: str, embed: Optional[Embed] = None):
        ...
        try:
            ...
            logger.error(f"Erro ao enviar mensagem para thread da raid {raid_id}: {e}")
            return None 
    ...
    async def voice_monitor(self):
        """Monitora canais de voz para manter sincronização - VERSÃO CORRIGIDA"""
        for boss_id, voice_channel_id in list(self.voice_queue_channels.items()):
            ...
            for user_id in raid['members']:
                # Usar Member em vez de User para acessar voice state
    ...
    async def complete_raid(self, raid_id: int, success: bool = True, interaction: discord.Interaction = None):
        ...
        if raid.get('is_voice_queue') and raid['boss_id'] in self.voice_queue_channels:
            ...
            if voice_channel:
                # Mover todos os membros para o canal original primeiro
                required_channel = self.bot.get_channel(REQUIRED_VOICE_CHANNEL_ID)
                if required_channel:
                    for member in voice_channel.members:
                        if not member.bot:
                ...
                try:
                    ...
                    logger.info(f"Canal de voz {voice_channel_id} deletado após conclusão da raid {raid_id}")
                except Exception as e:
                    logger.error(f"Erro ao deletar canal de voz {voice_channel_id}: {e}")
        ...
        for user_id in raid['members']:
            # Obter Member em vez de User para acessar voice state
            guild = self.bot.get_guild(raid['guild_id'])
            member = guild.get_member(user_id) if guild else None
        ...
        # Notificar na thread da raid se existir
        if raid_id in self.raid_threads:
            thread = self.bot.get_channel(self.raid_threads[raid_id])
            if thread:
                status_emoji = "🎉" if success else "❌"
                ...
                # Arquivar thread após 5 minutos
                await asyncio.sleep(300)
                try:
                    await thread.edit(archived=True, locked=True)
                    logger.info(f"Thread {thread.id} arquivada para raid {raid_id}")
                except:
                    pass
        ...
        # Limpar da memória após um tempo
        await asyncio.sleep(10)  # 5 minutos
        await self.cleanup_raid(raid_id)
