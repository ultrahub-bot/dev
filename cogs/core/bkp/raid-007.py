...
class RaidBuilderSystem(commands.Cog):
    ...
    async def cleanup_raid(self, raid_id: int):
        ...
        if raid_id in self.raid_threads:
            ...
            if thread:
                try:
                    await thread.delete(reason=f"Raid {raid_id} concluída/cancelada")
                    logger.info(f"Thread {thread_id} deleted for raid {raid_id}")
                except Exception as e:
    ...
    async def send_raid_embed(self, raid_data: Dict, channel: discord.TextChannel):
        ...
        try:
            ...
            thread_message += f"💬 Use esta thread para coordenar a raid!\n"
    ...
    async def handle_ready_check(self, interaction: discord.Interaction, raid_id: int):
        ...
        if len(raid['members']) < 3:  # Pelo menos 2 pessoas
            return await interaction.response.send_message(
                "❌ Precisa de pelo menos 3 jogadores para iniciar o ready check!",
                ephemeral=True
            )
